...
12 Oct 2025 - Successfully assigned the static IP to my server. First, I ran 'ip a' to verify the address, then 'ip route show default' to find my default gateway address. I ran 'ls /etc/netplan/' to see the file name, which I then edited, assigning it the 192.168.100.200/24 address. Minor obstacles with YAML text alignment were sorted quickly. Our server has a static ip address which was verified using 'ip a show enp0s3' command.
